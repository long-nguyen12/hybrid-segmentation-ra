...
def evaluate(model, dataloader, device):
    ...
    for images, labels in tqdm(dataloader):
        ...
        logits, score4, score3, score2 = model(images)
        preds = torch.sigmoid(score2)
        # logits = model(images)
...
def evaluate_msf(model, dataloader, device, scales, flip):
    ...
    ious, miou = metrics.compute_iou()
    return acc, macc, dice, mdice, ious, miou
...
def main(cfg):
    ...
    dataloader, trainset = create_dataloaders(cfg['DATASET']['ROOT'], 'val', eval_cfg['IMAGE_SIZE'], 1, 4)
