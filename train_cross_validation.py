...
def main(cfg, save_dir, train_loader, val_loader):
    ...
    for epoch in range(epochs):
        ...
        for iter, (img, lbl) in pbar:
            if epoch <= 1:
                optimizer.param_groups[0]["lr"] = (epoch * iter) / (1.0 * iters_per_epoch) * lr
            else:
                scheduler.step()
        ...
        if (epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 or (epoch+1) == epochs:
            miou, mdice, f1_score, mprecision, mrecall = evaluate(model, val_loader, device, "Training")
            ...
            print(f"mIoU = {miou}, mdice = {mdice}, f1_score = {f1_score}, precision = {mprecision}, mrecall = {mrecall}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='configs/five_fold.yaml', help='Configuration file to use')
    ...
    dataloader, dataset = create_dataloaders('data/data/Kvasir/', [352, 352], batch_size)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        print(f"Training Fold {fold + 1}... train: {len(train_loader)}, val: {len(val_loader)}")
        main(cfg, save_dir / f"fold_{fold}", train_loader, val_loader)
        fold += 1
